...
def run_fold_switch_pipeline(run_mode, foldpair_ids_to_run='ALL',output_dir ="Pipeline",
                             pdbids_file="data/foldswitch_PDB_IDs_full.txt", run_job_mode="inline", other_options=None):
...
#    print("START RunFoldSwitch PIPELINE!!")
    ...
    for foldpair_id in foldpair_ids_to_run:
        ...
        # try: # Change to try catch !!!
            ...
            if run_mode != "load":  # convert to fasta after loading
            ...
            if run_job_mode == "inline":
                print("Inline with other options: ", other_options)
                run_fold_switch_pipeline_one_family(run_mode, foldpair_id, pdbids[i], pdbchains[i],fasta_file_name, other_options)
            else:  # run job
                if run_mode == "load":
                    run_str = ''  # no pipeline in this mode !!!!
                    print("Enter load function:::")
                    print(i, cur_family_dir, foldpair_id, pdbids[i])
                    load_seq_and_struct(cur_family_dir, pdbids[i], pdbchains[i])
...
#                    print("plot clusters predictions:", plot_tree_clusters)
                    cmap_dists_vec[i], seqs_dists_vec[i], num_seqs_msa_vec[i], _ = \
                        make_foldswitch_all_plots(pdbids[i], output_dir, foldpair_id, pdbchains[i], plot_tree_clusters, global_plots=other_options.global_plots)
#                    print("Finished Running make_plot from outside333")
                ...
                if run_mode == 'Analysis':
                    run_str = "sbatch " + DATA_DIR + "/Analysis.sh " + foldpair_id
...
def run_fold_switch_pipeline_one_family(run_mode, foldpair_id, pdbids, pdbchains, fasta_file_name, other_options):
    run_str = ''
    cmap_dists_vec, seqs_dists_vec, num_seqs_msa_vec = [None]*3
    cur_family_dir = output_dir + "/" + foldpair_id
    if run_mode in ["load", "load_seq_struct"]:  #      if load_seq_and_struct or run_pipeline:  # also for entire pipeline
        run_str = ''  # no pipeline in this mode !!!!
        print("Loading inline, cur_family_dir: ", cur_family_dir, " foldpair_id: ", foldpair_id)
        load_seq_and_struct(cur_family_dir, pdbids, pdbchains)
    if run_mode == "get_msa":
    ...
    if run_mode == "run_esmfold":
        run_str = 'python3  ./ESMFoldHF.py  -input ' + foldpair_id
...
#       compute_global_and_residue_energies(pdb_pair_files, foldpair_ids_to_run, output_dir)
    ...
    if run_mode == "plot":  # Plotting, works only in windows?
        cmap_dists_vec, seqs_dists_vec, num_seqs_msa_vec, concat_scores = \
            make_foldswitch_all_plots(pdbids, output_dir, foldpair_id, pdbchains, plot_tree_clusters, global_plots=other_options.global_plots)
...
def parse_foldswitch_args():
    parser = argparse.ArgumentParser(description="FoldSwitch program")

    # Required argument: run_mode (e.g., 'plot', 'build-tree')
    parser.add_argument("run_mode", help="Operation mode (e.g., plot, build-tree)")

    # Optional positional argument: foldpair_ids
    parser.add_argument("foldpair_ids", nargs="?", default="ALL",
                        help="Specific fold pair IDs to run (e.g., 12dt). Default: ALL")

    # Optional flag arguments
    parser.add_argument("--global", dest="global_plots", action="store_true",
                        help="Enable global plots")

    parser.add_argument("--plot_trees", action="store_true",
                        help="Run tree plots")
...
#    return run_mode, foldpair_ids_to_run


######################################################################################################################
########################## Fold Switch Pipeline ######################################################################
######################################################################################################################
...
#        run_mode, foldpair_ids_to_run = parse_foldswitch_args()  # get user input fro the command line

        args = parse_foldswitch_args()

        # Access the arguments
        run_mode = args.run_mode
        foldpair_ids_to_run = args.foldpair_ids
        is_global = args.global_plots
...
#    if run_mode == "html": # New: generate html tables
#        html_tables = True
    ...
    res_DF = run_fold_switch_pipeline(run_mode, foldpair_ids_to_run,
                                      output_dir="Pipeline", pdbids_file="data/foldswitch_PDB_IDs_full.txt", run_job_mode=run_job_mode, other_options=args)
