...
parser.add_argument("--input_msa_folder", help="MSA to run prediction. First seq will be the predicted seq")
...
parser.add_argument("--af2_dir", default='./af_weights/', help="AlphaFold code and weights directory")
...
msas = os.listdir(args.input_msa_folder)
print(msas)
for msa in msas:
    if('a3m' in str(msa)):
        start = time.time()
        print(f'Get prediction for {msa} ...')
        name=os.path.basename(args.input_msa_folder + '/' + msa ).replace('.a3m','')
        features = make_processed_feature_dict(runner,args.input_msa_folder + '/' + msa, name=name, seed=args.seed)
        result = parse_results(runner.predict(features, random_seed=args.seed), features)
        write_results(result, args.output_dir+name+'.pdb')
        print(f'Finish to get prediction for {msa} !')
        print('Running  %s' % msa, time.time() - start)
