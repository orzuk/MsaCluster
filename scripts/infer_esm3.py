...
import os, sys, argparse, traceback, glob, gc
...
# Make CUDA allocator less fragmentation-prone and friendlier to many short runs
...
def run_hf_esmfold(sequence: str, device: str) -> str:
    ...
    # CUDA hygiene: less VRAM + fewer OOMs on L4
    if device == "cuda":
        torch.backends.cuda.matmul.allow_tf32 = True
        torch.set_float32_matmul_precision("medium")
        model = model.half()
    ...
    # Fast path some builds expose
    if hasattr(model, "infer_pdb"):
        try:
            with torch.inference_mode():
                with amp_ctx:
                    pdb_txt = model.infer_pdb(sequence)
            if isinstance(pdb_txt, str) and "ATOM" in pdb_txt:
                # cleanup
                if device == "cuda":
            ...
            # fall through to manual tokenization path
            pass
    ...
    # Move tensors; only half the floating tensors (keep indices long)
    for k, v in list(batch.items()):
        if isinstance(v, torch.Tensor):
            v = v.to(device, non_blocking=True)
            if device == "cuda" and v.dtype in (torch.float32, torch.float64, torch.bfloat16):
                v = v.half()
            batch[k] = v

    # Forward pass with AMP/inference_mode
    with torch.inference_mode():
        with amp_ctx:
            out = model(**batch)

    # Convert to PDB BEFORE cleanup
    ...
    if callable(to_pdb):
        ...
        if isinstance(pdb_txt, str) and "ATOM" in pdb_txt:
            return pdb_txt
        raise RuntimeError("[hf-esmfold] Conversion returned non-PDB text.")

    # If we got here, there was no converter
    del out, batch
    if device == "cuda":
        torch.cuda.empty_cache()
    gc.collect()
