...
#           comparison = compare_residues_energies(residue_energies[0], residue_energies[1])
...
def compute_deltaG_and_residue_energies2(pdb_path: str, log_file_path: str = None):
    """
    Compute global ΔG and per-residue energies using PyRosetta, with logging.
    ...
    Returns:
    - float: Global ΔG.
    - list of tuples: Per-residue energies [(residue_name, residue_index, residue_energy)].
    """
    if log_file_path:
        with open(log_file_path, "w") as log_file:
            with RedirectStdStreams(stdout=log_file, stderr=log_file):
                return _compute_deltaG_and_residue_energies_internal(pdb_path, log_file)
    else:
        return _compute_deltaG_and_residue_energies_internal(pdb_path, sys.stdout)


def _compute_deltaG_and_residue_energies_internal(pdb_path: str, log_output):
    """
    Internal helper to compute ΔG and residue energies, logging to the specified output.

    Parameters:
    - pdb_path (str): Path to the PDB file.
    - log_output (file or stream): Log output destination.
    ...
    log_output.write(f"Processing file: {pdb_path}\n")
    pose = pyrosetta.pose_from_file(pdb_path)
    scorefxn = pyrosetta.get_fa_scorefxn()

    log_output.write("Calculating global ΔG...\n")
    deltaG = scorefxn(pose)
    log_output.write(f"Global ΔG: {deltaG}\n")

    log_output.write("Calculating per-residue energies...\n")
    energies = pose.energies()
    residue_energies = [
        (pose.residue(i).name(), i, energies.residue_total_energy(i))
        for i in range(1, pose.total_residue() + 1)
    ...
    for residue_name, residue_index, residue_energy in residue_energies:
        log_output.write(f"Residue {residue_name} {residue_index}: {residue_energy}\n")

    return deltaG, residue_energies


def compare_residues_energies(residues1, residues2):
    """
    Compare per-residue energies between two structures.

    Parameters:
    - residues1 (list): Residue energies from the first structure.
    - residues2 (list): Residue energies from the second structure.

    Returns:
    - list of tuples: Comparison [(residue_name, index, energy1, energy2, difference)].
    """
    comparison = []
    for res1, res2 in zip(residues1, residues2):
        if res1[1] == res2[1]:  # Matching residue index
            diff = res1[2] - res2[2]
            comparison.append((res1[0], res1[1], res1[2], res2[2], diff))
    return comparison
...
# Compute contribution of every Amino-Acid to free energy
def compute_residue_contributions(pdb_path):
    # Load the structure
    pose = pyrosetta.pose_from_file(pdb_path)
    ...
    energies = pose.energies()
    residue_scores = []
    for i in range(1, pose.total_residue() + 1):  # Rosetta residues are 1-indexed
        residue_score = energies.residue_total_energy(i)
        residue_scores.append((pose.residue(i).name(), i, residue_score))
