...
import pandas as pd
import re
main_folder = '/Users/steveabecassis/Desktop/Pipeline_res'
#output_msa_cluster
fold_pair_nb = [i[:-5] for i in os.listdir(f'{main_folder}/HTMLs_new_2110')]
...
for fold_pair in fold_pairs:
    print(fold_pair)
    cluster_count = len(os.listdir(f'{folder}/{fold_pair}/output_msa_cluster'))
    af_pred_txt = os.listdir(f'{folder}/{fold_pair}/AF_preds')
    AF_pred_count = max(len( [i for i in af_pred_txt if 'txt' in i]) - 2 , 0)
    cmap_count = len(os.listdir(f'{folder}/{fold_pair}/output_cmap_esm')) - 2
    cmap_viz_count = len([i for i in os.listdir(f'{folder}/{fold_pair}/output_cmap_esm/VizCmaps') if 'tol_0' in i])/2
...
df[df['have_notebook'] == True].count()
df[df.AF_pred_count == 0][df['have_notebook'] == True]
